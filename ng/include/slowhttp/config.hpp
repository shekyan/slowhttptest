...
struct Config {
  ...
  // Cookie plus every -1 header, already CRLF-terminated and ready to splice into
  // a request. Built here rather than in each attack so the probe cannot end up
  // addressing a different endpoint than the attack does -- with host-routing or
  // tenant headers in play, that would silently measure the wrong service.
