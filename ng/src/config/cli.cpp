...
void print_usage() {
  // One line per option, near enough. The detail lives in slowhttptest-ng(1):
  // this grew to 96 lines at one point, which pushed the slow-read options off
  // the bottom of an ordinary terminal and made them look absent.
  std::printf(
  ...
  std::printf(
      ...
      "  -1 header     extra header, repeatable; also sent on the probe (--header)\n"
      "  -P data       request body for -B; @file to read from a file (--data)\n"
      "  -A ua         User-Agent; the tool identifies itself by default\n"
      "  -v level      verbosity 0-4 (1); 4 breaks down connect failures by cause\n"
      "  -q            quiet: no console output at all (--quiet)\n"
      "  -h            this help (--help)\n"
      "  -V            version, project URL, TLS and connection ceiling (--version)\n"
      "  --connect-timeout SEC   drop a connection not established in this long\n"
      "                and reuse the slot (10); 0 leaves it to the OS\n"
      ...
      "  --no-probe              do not measure availability (no verdict, no report)\n"
      "\n"
      ...
      "                Affects the gate only; a 503 is still a served response\n"
      "\n"
      "Range (-R) options:\n"
